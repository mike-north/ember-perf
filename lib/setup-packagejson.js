...
if (['ember-1.5', 'ember-1.6', 'ember-1.7', 'ember-1.8', 'ember-1.9'].indexOf(process.env.EMBER_TRY_SCENARIO) >= 0) {
  var pkgJson = JSON.parse(fs.readFileSync('package.json'));
  fs.writeFileSync('package.json.bak', JSON.stringify(pkgJson, null, 2));
  ...
  pkgJson.name = pkgJson.name + '-handlebars';
